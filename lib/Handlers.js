...
class TusHandlers {
  ...
  handle(req, res, options) {
    ...
    if (this.handlers[req.method]) {
      return co(this.handlers[req.method].send(req, res, options))
        .then((end) => {
          let success = Promise.resolve(null);
          console.log('Handled stuff');
          ...
          return success.then((result) => {
            console.log(result);
            console.log(res.finished, 'before')
            end();
            console.log(res.finished, 'after')
          }, (err) => {
            // End the response first and bubble up the error
            end();
            throw err;
          });
        });
